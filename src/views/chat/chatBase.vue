...
import { NButton, NInput, NTooltip, useDialog, useMessage, NSwitch, NPopover, NCascader } from 'naive-ui'
...
async function queryAppDetail(id: number){
	const res = await fetchQueryOneCatAPI({id})
  ...
  <div class="h-full  flex flex-col bg-white dark:bg-[#111114]">
    ...
    <main class="flex-1 overflow-hidden">
      <div id="scrollRef" ref="scrollRef" class="relative h-full overflow-hidden overflow-y-auto scroll-smooth">
        ...
        >
          <template v-if="!dataSources.length && !activeAppId">
            ...
            <div class="flex justify-center items-center">
              <AppTips :appInfo="appDetail"  @prompt="handlePrompt" />
            </div>
          </template>
		...
		<footer>
			<div :class="['m-auto max-w-screen-4xl', isMobile ? 'px-2 py-1' : 'px-4 py-2']">
						...
						<div class="absolute bottom-2 left-2 right-2">
							<div class="flex items-center justify-between">
								<div class="flex space-x-2">
									...
									<n-popover style="width: 200px"  raw :show-arrow="false" v-if="chatStore.chatPreList?.length">
										<template #trigger>
											<NTooltip trigger="hover" placement="right" :disabled="isMobile">
												<template #trigger>
													<button class="flex h-8 w-8 items-center justify-center rounded border transition hover:bg-[#eef0f3] dark:border-neutral-700 dark:hover:bg-[#33373c]" @click="openChatPre">
														<span><SvgIcon class="text-lg" style="width: 1em;height: 1em" icon="mingcute:book-line" /></span>
													</button>
												</template>
												快问模板
											</NTooltip>
								...
								<div class="flex justify-between items-center">
									<div class="flex items-center text-neutral-400 cursor-pointer hover:text-[#3076fd] "  >
										...
										<!-- <n-switch size="small" /> -->
										<div class="mx-4 h-full text-neutral-300 dark:text-neutral-600">|</div>
									</div>
